...
// PRÉVISITE (readForTender). « Aide-moi à répondre à cet appel d'offre. » Zéro IA :
// on restitue la matière captée sur site, organisée pour chiffrer. La couche IA
// (« voilà ce que j'ai compris », puis mémoire technique) viendra par-dessus, gated.
// Prototype : accessible par URL, pas encore dans la navigation. Cf. [[continuite-operationnelle-2026-05-22]].
...
export default async function AoReadingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-3xl space-y-6 py-6">
      ...
      {/* Continuer la collecte sur le terrain (mobile). La prévisite = une visite. */}
      <Link
        href={`/m/site/${id}`}
