...
const PHASE_FR: Record<string, string> = { prospect: 'Prospect', en_ao: 'En appel d’offre' }
    ...
    <div className="max-w-3xl space-y-8 py-6">
      <header className="space-y-1">
        <h1 className="text-2xl font-semibold inline-flex items-center gap-2">
          <Compass className="h-6 w-6 text-sky-600" /> Opportunités
        </h1>
        <p className="text-sm text-muted-foreground">
          Un dossier d&apos;opportunité naît dès la prévisite — avant le contrat. S&apos;il est gagné,
          le même dossier devient un chantier sans rien perdre de sa mémoire.
        </p>
      </header>
      ...
      <form action={createProspectAction} className="space-y-3 rounded-2xl border bg-card p-4">
        <h2 className="text-sm font-semibold">Nouvelle prévisite</h2>
      ...
      {opportunities.length === 0 ? (
        <p className="rounded-xl border border-dashed bg-muted/20 px-4 py-8 text-center text-sm text-muted-foreground">
          Aucune opportunité en cours. Créez-en une ci-dessus avant de partir en prévisite.
        </p>
      ) : (
