...
function VisitSummary({ site, orgLabels }: { site: SiteImpact; orgLabels: OrgLabels }) {
  const metrics = [
  ...
  return (
    <section className={`${surface} p-5 sm:p-6`}>
      ...
      <Link href={`/sites/${site.siteId}`} className="mt-5 inline-flex items-center gap-2 text-xs font-semibold text-[#1463e8]">Voir le chantier <ArrowRight className="h-3.5 w-3.5" /></Link>
    </section>
  )
}
...
export function DashboardPremium({ firstName, orgNames, attention, visit, upcoming, sites, aSavoir, orgLabels }: Props) {
  const site = visit.sites[0]
  return <div className="min-h-screen bg-[#f8fafc] px-1 pb-12 pt-1 sm:px-2"><div className="mx-auto max-w-[1240px] space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-2"><SitesTable sites={sites} /><PriorityActions items={[...attention.red, ...attention.orange]} orgLabels={orgLabels} /></div>
