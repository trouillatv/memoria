...
const STATUS_LABELS: Record<string, string> = {
  planned: 'Planifiée', in_progress: 'En cours', completed: 'Terminée', validated: 'Validée', skipped: 'Annulée',
}
...
export default async function MissionsPage() {
  ...
  const { data: interventions } = await supabase
    .from('interventions')
    .select(`
      id, scheduled_at, status, mission_id,
...
function InterventionRow({ item }: { item: { id: string; scheduled_at: string; status: string; mission?: { name: string; site?: { name: string; contract?: { id: string; name: string; client_name: string } | null } | null } | null } }) {
  const date = new Date(item.scheduled_at)
  const dateLabel = date.toLocaleDateString('fr-FR', { weekday: 'short', day: '2-digit', month: 'short' })
  const timeLabel = date.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })
  return (
    <li className="rounded border p-3 bg-card">
      <Link href={`/interventions/${item.id}`} className="flex items-start justify-between gap-3 -m-3 p-3 hover:bg-muted/20 rounded transition-colors">
        <div className="min-w-0 flex-1">
          ...
          <div className="text-sm">{item.mission?.name ?? '—'}</div>
