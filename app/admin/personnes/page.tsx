...
import { getUsersActivitySummary, type UserActivity } from '@/lib/db/admin-monitoring'
...
import { CreateOrgForm, MoveUserOrgForm } from '../organisations/OrgForms'
...
const STATUS_DOT: Record<UserActivity['status'], string> = {
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/30 text-xs uppercase tracking-wider text-muted-foreground">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left">Mdp</th>
                ...
                {users.length === 0 ? (
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-muted/20">
                      ...
                      <td className="px-3 py-2">
                        <MoveUserOrgForm userId={u.id} currentOrgId={oid} orgs={orgList} />
                        {oid && !orgNameById.has(oid) && (
                      ...
                      <td className="px-3 py-2">
                        <span className="inline-flex items-center gap-1.5 whitespace-nowrap text-xs text-muted-foreground">
                          <span className={`inline-block h-1.5 w-1.5 rounded-full ${STATUS_DOT[act?.status ?? 'inactive']}`} />
                          {relativeConnexion(act?.last_activity_at ?? null)}
                        </span>
                      </td>
