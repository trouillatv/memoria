...
export function attentionItemToMemorySignal(
  ...
  return {
    ...
    facts: [
      ...
      { type: 'attention_where', key: 'where', value: item.where, confidence: null, sourceIds: meta.sources.map((source) => source.id), detectedAt: now, occurredAt: null, dueAt: null, validUntil: null },
    ],
...
export function nowItemToMemorySignal(
  ...
  return {
    ...
    origin: 'rules',
    facts: [
      { type: item.sourceType, key: 'title', value: item.title, confidence: null, sourceIds: [item.id], detectedAt: now, occurredAt: item.sourceType === 'passage' ? item.startsAt : null, dueAt: item.sourceType === 'passage' ? null : item.dueDate, validUntil: null },
      { type: 'dashboard_item', key: 'source_type', value: item.sourceType, confidence: null, sourceIds: [item.id], detectedAt: now, occurredAt: null, dueAt: null, validUntil: null },
      { type: 'dashboard_item', key: 'site_name', value: item.siteName, confidence: null, sourceIds: [item.siteId], detectedAt: now, occurredAt: null, dueAt: null, validUntil: null },
      { type: 'dashboard_item', key: 'priority', value: item.priority, confidence: null, sourceIds: [item.id], detectedAt: now, occurredAt: null, dueAt: null, validUntil: null },
      { type: 'dashboard_item', key: 'action_id', value: item.actionId, confidence: null, sourceIds: [item.id], detectedAt: now, occurredAt: null, dueAt: null, validUntil: null },
      { type: 'dashboard_item', key: 'can_complete', value: item.canComplete, confidence: null, sourceIds: [item.id], detectedAt: now, occurredAt: null, dueAt: null, validUntil: null },
      { type: 'dashboard_item', key: 'starts_at', value: item.startsAt, confidence: null, sourceIds: [item.id], detectedAt: now, occurredAt: null, dueAt: null, validUntil: null },
    ],
