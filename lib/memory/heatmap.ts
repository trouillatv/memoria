...
export async function getMemoryHeatmap(days = 84): Promise<HeatmapCell[]> {
  ...
  const [acks, shares, notes, anomalies] = await Promise.all([
    sb.from('handover_briefs').select('acknowledged_at').gte('acknowledged_at', sinceIso).not('acknowledged_at', 'is', null),
    sb.from('handover_briefs').select('shared_at').gte('shared_at', sinceIso).not('shared_at', 'is', null),
    sb.from('site_notes').select('created_at').eq('kind', 'a_savoir').is('deleted_at', null).gte('created_at', sinceIso),
    sb.from('intervention_anomalies').select('created_at').eq('status', 'open').gte('created_at', sinceIso),
  ])
