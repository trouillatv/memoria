...
export async function getActorsGraph(orgIds: string[], opts?: { unfiltered?: boolean }): Promise<ActorsGraph> {
  ...
  const [contactRes, tfmRes, missionRes, castingRes, actionRes, aliasMap] = await Promise.all([
  ...
  // P0-3A : ces trois requêtes lisent des FK company_id brutes, indépendamment de
  // getActorsCockpit ci-dessus — sans résolution, une arête pourrait pointer vers
  // l'id alias alors que le nœud entreprise correspondant (issu du cockpit, déjà
  // canonicalisé) n'existe plus sous cet id.
  const canon = (id: string) => resolveCanonicalCompanyId(aliasMap, id)

  const companyIdByContact = new Map(
    ((contactRes.data ?? []) as Array<{ id: string; company_id: string | null }>).map((c) => [c.id, c.company_id ? canon(c.company_id) : null]),
  )
  ...
  return buildActorsGraph({
    ...
    casting: ((castingRes.data ?? []) as Array<{ company_id: string; site_id: string; effective_from: string | null }>).map((r) => ({ companyId: canon(r.company_id), siteId: r.site_id, effectiveFrom: r.effective_from })),
    openActions: ((actionRes.data ?? []) as Array<{ id: string; title: string; site_id: string; assigned_contact_id: string | null; assigned_company_id: string | null; due_date: string | null; created_at: string | null }>)
      .map((r) => ({ id: r.id, title: r.title, siteId: r.site_id, contactId: r.assigned_contact_id, companyId: r.assigned_company_id ? canon(r.assigned_company_id) : null, overdue: !!r.due_date && r.due_date < today, createdAt: r.created_at })),
  })
}
