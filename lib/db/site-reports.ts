...
// ── Statut chantier du pipeline mémoire (post-import) ───────────────────────
// Sous-lot 4, stabilisation UX Points/Suivi (mandat Vincent 2026-09-17) :
// un signal dérivé des colonnes déjà écrites par
// lib/subjects/historical-import-post-processing.ts sur `site_reports`,
// jamais d'un minuteur. Réutilisé par les en-têtes Suivi/Points et par la
// liste « Chantiers à surveiller » d'Aujourd'hui — jamais recalculé
// localement par chaque surface.

export interface SiteMemoryBuildStatus {
  /** Une étape (canonicalisation ou analyse de similarité) est démarrée sans être terminée ni en échec. */
  isProcessing: boolean
  /** Une étape a laissé une erreur non résolue sur le rapport le plus récent concerné. */
  hasError: boolean
  siteReportId: string | null
}

interface MemoryBuildPipelineRow {
  id: string
  site_id?: string
...
const MEMORY_BUILD_PIPELINE_COLUMNS =
  'id, site_id, canonical_reconcile_started_at, canonical_reconciled_at, canonical_reconcile_error, ' +
  'similarity_analysis_started_at, similarity_analysis_completed_at, similarity_analysis_error, ' +
  'action_cbo_reconcile_error'

function computeMemoryBuildStatus(rows: MemoryBuildPipelineRow[]): SiteMemoryBuildStatus {
  for (const row of rows) {
    const hasError = !!(row.canonical_reconcile_error || row.similarity_analysis_error || row.action_cbo_reconcile_error)
    if (hasError) return { isProcessing: false, hasError: true, siteReportId: row.id }
    const canonicalPending = !!row.canonical_reconcile_started_at && !row.canonical_reconciled_at
    const similarityPending = !!row.similarity_analysis_started_at && !row.similarity_analysis_completed_at
    if (canonicalPending || similarityPending) return { isProcessing: true, hasError: false, siteReportId: row.id }
  }
  return { isProcessing: false, hasError: false, siteReportId: null }
}

/** Signal chantier-level du pipeline post-import, pour un seul site (en-têtes Suivi/Points). */
export async function getSiteMemoryBuildStatus(siteId: string): Promise<SiteMemoryBuildStatus> {
  const supabase = createAdminClient()
  const { data, error } = await supabase
    .from('site_reports')
    .select(MEMORY_BUILD_PIPELINE_COLUMNS)
    .eq('site_id', siteId)
    .order('created_at', { ascending: false })
    .limit(5)
  if (error) throw error
  return computeMemoryBuildStatus((data ?? []) as unknown as MemoryBuildPipelineRow[])
}

/** Même signal que ci-dessus, en un seul aller-retour pour plusieurs sites (liste Aujourd'hui). */
export async function getSitesMemoryBuildStatus(siteIds: string[]): Promise<Map<string, SiteMemoryBuildStatus>> {
  const result = new Map<string, SiteMemoryBuildStatus>()
  ...
  const { data, error } = await supabase
    ...
    .order('created_at', { ascending: false })
  if (error) throw error
  const rowsBySite = new Map<string, MemoryBuildPipelineRow[]>()
  for (const row of (data ?? []) as unknown as MemoryBuildPipelineRow[]) {
    if (!row.site_id) continue
    const list = rowsBySite.get(row.site_id) ?? []
    list.push(row)
    rowsBySite.set(row.site_id, list)
  }
  for (const siteId of uniqueIds) {
    result.set(siteId, computeMemoryBuildStatus(rowsBySite.get(siteId) ?? []))
  }
  return result
}
