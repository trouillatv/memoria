...
2b. Un item de type « Briefing sur X », « Rappel de X », « Présentation de X », « Test de X », « Démonstration de X » qui décrit un contenu COUVERT PENDANT la visite (partie d'un déroulé de séance, d'un ordre du jour, d'un rappel fait en début ou en cours de rendez-vous) n'est PAS une action future — même sous forme de bullet nominal sans verbe. Ne créer une **action** que si le document formule explicitement une obligation restante, un responsable, une échéance, ou une tournure prospective claire (« à refaire », « à programmer », « prévoir un prochain… », « avant le… »). En cas de doute sur le moment (pendant vs après la visite), rester conservateur et ne pas créer d'action.
2c. Symétrie constat positif/négatif : une amélioration, une correction, un test réussi ou la bonne exécution d'une procédure qui répond à un problème précédemment observé sur ce chantier est un ÉTAT constaté au même titre qu'une défaillance — produire une proposition **observation** (ou **knowledge_fact**, thematic_category='test_control', si elle documente un fait durable) même si l'issue est positive. Ne jamais omettre un constat au seul motif qu'il est positif ou rassurant. Cette proposition ne doit jamais, à elle seule, faire passer un autre problème du même sujet à statusAtDocumentDate='réalisé' — une amélioration démontrée coexiste avec les défaillances résiduelles encore actives sur le même sujet, elle ne les résout pas automatiquement.
2d. Résultat significatif d'un test, exercice, contrôle ou mise en situation — même sans défaillance préalable : un résultat significatif de test, exercice, contrôle ou visite doit pouvoir être conservé comme preuve d'évolution lorsqu'il renseigne l'état ou la capacité réelle du système, même si le résultat est positif — y compris lorsqu'il ne répond à AUCUNE défaillance précédemment observée (contrairement à 2c, qui suppose un problème antérieur, 2d couvre aussi le déroulé d'un exercice/test réalisé pour la première fois dans ce document, ex. une mise en situation, un exercice d'évacuation, une simulation d'appel aux secours). Chaque étape franchie, réalisée ou déclenchée d'un tel déroulé (chrono lancé, levée de doute effectuée, appel simulé passé, alarme/évacuation déclenchée à un instant précis, délai mesuré) est un constat à conserver au même titre qu'un manquement du même déroulé — produire une proposition **observation** (le déroulé constaté) ou **knowledge_fact** (thematic_category='test_control') si le fait a une valeur durable de connaissance sur le site. Ne JAMAIS convertir un « test réussi » en statusAtDocumentDate='réalisé'/'resolved' pour un autre sujet du même thème — un résultat de test s'ajoute aux constats existants, il ne clôt jamais automatiquement un problème tiers (cf. 2c).
Ce résultat n'a de valeur de suivi, et ne doit être extrait, que s'il remplit AU MOINS UNE des conditions suivantes :
  (a) il provient explicitement d'un test, exercice, contrôle ou visite désigné comme tel par le document (jamais une activité courante non testée) ;
  (b) il porte sur une capacité ou un système suivi dans la durée sur ce chantier (alarme, évacuation, procédure de sécurité, équipement technique, compétence d'un intervenant) ;
  (c) il permet un constat d'amélioration, de dégradation ou de maintien par rapport à un état antérieur (documenté ou implicite, ex. un exercice similaire déjà mené) ;
  (d) il répond à une anomalie ou un point de vigilance habituellement suivi sur ce type de chantier (délai d'évacuation, remontée d'alarme, appel aux secours…).
Ne PAS créer de proposition pour un fait positif anodin sans valeur de suivi (ex. « la réunion s'est bien déroulée », un détail positif sans rapport avec une capacité ou un système suivi) : cette règle ne devient JAMAIS « extraire tout événement positif » — elle cible spécifiquement les résultats de test/exercice/contrôle qui renseignent une capacité réelle.
3. Conserver les formulations incertaines (« à vérifier », « à confirmer », « semble ») dans le label ou la description.
3c. Une prescription assortie d'une CONDITION ou d'une MODALITÉ explicite (« si… », « non obligatoire », « le cas échéant », « à envisager », « option », « recommandé sans obligation », « décision à prendre ») doit conserver cette nuance jusque dans le **label** lui-même, pas seulement dans la description — ne jamais reformuler une prescription conditionnelle ou optionnelle en obligation ferme (ex. ne pas transformer « Prévoir un désenfumage naturel si la DM n'excède pas 30 m (non obligatoire) » en « Prévoir un désenfumage naturel » sans la réserve). Si le document présente explicitement PLUSIEURS options entre lesquelles un arbitrage reste à faire, ou un choix/décision effectivement acté lors de la visite → utiliser la famille **decision** existante plutôt qu'une action ferme ; ne jamais créer de nouvelle famille pour cela. Un doute exprimé sur la nécessité ou l'opportunité d'une action UNIQUE (« semble nécessaire », « pourrait être utile », « il faudrait », « serait souhaitable », « à envisager ») n'est PAS un choix entre options ni une décision actée : cela reste une **action**, avec l'incertitude conservée dans le label (cf. règle 3) — ne pas basculer en decision au seul motif que la formulation est hédée.
...
- **action** : une CHOSE À FAIRE explicitement demandée par le document — prescription, consigne, préconisation, item d'une liste « actions à réaliser / à prioriser », verbe d'action à l'infinitif ou à l'impératif (vérifier, transmettre, mettre en place, reprendre, organiser, modifier, récupérer, installer, déplacer, évacuer, sécuriser, former…). Une action reste une action **même sans responsable ni date**, et **quelle que soit la colonne ou la section** où elle figure (« Préconisations », « Remarques », « Conclusions », ou une phrase descriptive). Le responsable et la date sont des enrichissements facultatifs (cf. doctrine §8), jamais une condition d'existence. Lorsque le responsable nommé correspond à une proposition 'person' ou 'company' du même document, renseigner sourcePayload.linkedActorTemporaryKey avec la temporaryKey exacte de cette proposition. Utiliser uniquement une temporaryKey réelle produite dans ce même run. Ne jamais inventer une clé, ne jamais relier par proximité de page ou de nom : soit la clé existe dans les propositions du run, soit le champ est absent. Pour la famille action : renseigner sourcePayload.statusAtDocumentDate uniquement si l'état de la tâche est explicitement établi par le document — **jamais par défaut**. Le simple fait qu'une action soit nouvellement listée, pas encore mentionnée comme réalisée, ou qu'aucun statut ne soit indiqué, n'est PAS une preuve d'état « ouvert » : "ouvert" n'est jamais une valeur par défaut appliquée à toute action non résolue. N'émettre "ouvert" que si le document qualifie EXPLICITEMENT la tâche de non soldée, en attente ou restant à faire (« reste à réaliser », « en attente », « non traité », « reporté », « toujours à faire »). En l'absence de toute mention explicite de statut dans le document, laisser sourcePayload.statusAtDocumentDate absent — un statut absent est le résultat normal et attendu pour une action simplement listée sans indication d'état. Utiliser le vocabulaire canonique : "en cours" pour une action déclarée en progression, "ouvert" pour une action explicitement qualifiée de non soldée. Ne jamais émettre "à faire" comme valeur. N'émettre un état terminal tel que "réalisé" que si la tâche est entièrement soldée, sans réserve, attente ou reprise associée. "réalisé … non conforme", "réalisé … reprise à faire" ou "réalisé … en attente" ne prouvent jamais la résolution — laisser absent. Un VISA ou visa de plan ne prouve jamais à lui seul l'achèvement de la tâche physique.
- **decision** : décision structurante prise lors de la visite. Lorsque le décisionnaire nommé correspond à une proposition 'person' ou 'company' du même document, renseigner sourcePayload.linkedActorTemporaryKey avec la temporaryKey exacte de cette proposition. Utiliser uniquement une temporaryKey réelle produite dans ce même run. Ne jamais inventer une clé, ne jamais relier par proximité de page ou de nom : soit la clé existe dans les propositions du run, soit le champ est absent. Pour la famille decision : renseigner sourcePayload.statusAtDocumentDate uniquement si l'état d'EXÉCUTION de la décision (pas son statut de validation/adoption) est explicitement établi par le document — **jamais par défaut**. Une décision nouvellement actée dont le document ne dit rien de l'exécution n'est PAS pour autant "ouverte" : "ouvert" n'est jamais une valeur par défaut appliquée à toute décision non soldée. N'émettre "ouvert" que si le document qualifie EXPLICITEMENT l'exécution de non soldée, en attente ou restant à mettre en œuvre. En l'absence de toute mention explicite de l'état d'exécution, laisser sourcePayload.statusAtDocumentDate absent. Utiliser le vocabulaire canonique : "en cours" pour une exécution en progression, "ouvert" pour une décision dont l'exécution est explicitement non soldée. Ne jamais émettre "à faire" comme valeur. N'émettre un état terminal tel que "réalisé" que si la décision est ENTIÈREMENT exécutée, sans réserve, attente, reprise, contrôle ou confirmation restante concernant le même sujet — jamais sur la seule présence d'un mot ("réalisé", "fait", "terminé", "remplacé", "réparé", "mis en service", "corrigé", "levé"). "réalisé … réserves à lever", "réalisé … contrôle/test à effectuer", "réalisé partiellement" ou "semble réalisé, à confirmer" ne prouvent jamais la résolution — utiliser "en cours" ou laisser absent selon le cas. Cette évaluation porte sur l'exécution constatée à la date du document, indépendamment de tout statut de workflow (proposée/actée/appliquée) de la décision elle-même.
...
**Rubrique « Présents » — application uniforme, quelle que soit la syntaxe qui suit le nom :** dès qu'un nom figure comme ITEM d'une liste, d'un tableau ou d'une énumération placée sous une rubrique/en-tête « Présents » ou « Étaient présents », cette personne est présente — peu importe ce qui suit son nom dans cet item : un tiret suivi d'une société (« Jean Dupont - Sté X »), un deux-points suivi d'une qualification, d'un diplôme ou d'une fonction (« Jean Dupont : SSIAP 2 », « Jean Dupont : chef d'équipe »), une parenthèse, ou aucune précision du tout. La syntaxe qui suit le nom à l'intérieur de cet item ne retire JAMAIS la preuve de présence apportée par le fait d'être listé sous cette rubrique — ne pas exiger en plus une case cochée ou la mention littérale « présent(e) » pour ces personnes-là. Cette règle ne s'applique qu'aux noms qui sont des ITEMS de la liste/rubrique elle-même : une personne seulement ÉVOQUÉE ailleurs dans le texte (citée en passant, mentionnée comme auteur d'un document, désignée dans une autre phrase) sans figurer comme item de la rubrique « Présents » reste "inconnu".

**Portée de la rubrique « Présents » — où s'arrête la liste :** la liste/rubrique « Présents » ou « Étaient présents » s'arrête au prochain intitulé de section/rubrique du document (nouvel en-tête explicite, ou changement de rubrique visible dans la mise en page/le cartouche : « Rappel préalable », « Ordre du jour », « Constats », « Mise en situation », « Débrief », etc.). Seuls les noms qui figurent EN TANT QU'ITEMS entre l'en-tête « Présents » et cette frontière appartiennent à la liste. Un nom qui apparaît APRÈS cette frontière — y compris sous une forme syntaxique proche de celle des items de la liste (nom suivi d'une parenthèse, d'un deux-points, d'une qualification) — n'appartient PAS à la liste « Présents », même si la ressemblance de forme est forte : ne jamais élargir la portée de la rubrique par ressemblance syntaxique, seule la POSITION du nom avant la frontière de section prouve l'appartenance à la liste. Exemple : si la rubrique « Présents » cite plusieurs noms puis le document change de rubrique (ex. « Rappel préalable »), et qu'un autre nom est mentionné plus loin dans une section différente (ex. « Constats ») sous une forme proche (« Question à [Nom] (fonction) : … »), seuls les noms de la liste initiale sont "présent" ; ce nom mentionné plus loin, hors de la liste, reste "inconnu" — la simple similitude de syntaxe (nom + parenthèse) ne suffit jamais à le rattacher rétroactivement à la rubrique « Présents ».
