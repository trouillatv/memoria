-- Migration 261 — Correction de started_at sur les visites historiques importées.
--
-- Bug : le RPC materialize_historical_visit ne peuplait pas started_at sur site_reports.
-- La page visite utilise `started_at ?? created_at` → affichait la date d'aujourd'hui
-- au lieu de la date du PV.
--
-- Fix : (1) recréer la fonction avec started_at = p_visit_date::timestamptz,
--       (2) corriger les visites déjà créées via leur source_document_id.

-- ── 1. BACKFILL des visites existantes ──────────────────────────────────────

UPDATE public.site_reports sr
SET    started_at = d.effective_date::timestamptz
FROM   public.documents d
WHERE  sr.source_document_id = d.id
