-- Migration 273 — Correction colonnes site_actions dans materialize_historical_visit()
--
-- Erreur : column "description" of relation "site_actions" does not exist
-- Cause  : le bloc WHEN 'action' de la migration 272 utilise des noms de colonnes
--          qui n'existent pas dans public.site_actions :
--            organization_id → absente de site_actions
--            description     → doit être "body"
--            deadline        → doit être "due_date"
--            issued_by       → doit être "assigned_to"
...
COMMENT ON FUNCTION public.materialize_historical_visit IS
  ...
  '(273 : correction noms colonnes site_actions — body/due_date/assigned_to)';
