-- Migration 272 — Lier automatiquement le document PV à la visite historique matérialisée
--
-- Problème :
--   Le PV historique est lié au chantier (document_links.target_type = 'site')
--   mais pas à la visite créée (site_reports). Résultat :
--   · le document n'apparaît pas dans l'écran final de la visite
--   · il apparaît dans la liste documents du chantier mais sans lien explicite avec la visite
--
-- Solution :
...
BEGIN
  ...
  -- ── Lier le document PV à la visite (pour qu'il apparaisse dans l'écran final) ──
  ...
  LOOP
    ...
    CASE rec.proposal_family
      ...
      WHEN 'observation' THEN
        -- observation vigilance → site_watchpoints
    ...
    -- Marquer la proposition comme matérialisée
    UPDATE public.document_extraction_proposal
      SET review_status = 'materialized'
      WHERE id = rec.id;
  END LOOP;

  -- ── Détecter si des knowledge_fact sont en attente (pipeline TypeScript) ──
  SELECT EXISTS (
    SELECT 1
      FROM public.document_extraction_proposal
      WHERE extraction_run_id = p_run_id
  ...
  -- Statut du run : materialized si tout est traité, partially_materialized sinon
