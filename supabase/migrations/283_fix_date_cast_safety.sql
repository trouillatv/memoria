-- Migration 283 — Cast de date sécurisé dans materialize_historical_visit().
--
-- Problème : le LLM peut produire des valeurs non-ISO pour dueDate
-- (ex : "25 du mois en cours", "fin de mois"…).
-- Le cast direct ::date plante alors avec "invalid input syntax for type date".
--
-- Correction : remplacer le test IS NOT NULL par un filtre regex qui n'accepte
-- que les formats ISO (YYYY-MM-DD) avant le cast. Toute autre valeur → NULL.
--
-- Champs concernés :
--   action   : dueDate (ligne 157-161 de mig 282)
--   deadline : dueDate (ligne 237-241 de mig 282)
--   decision : date_decision (ligne 195-198 de mig 282, même risque)
...
BEGIN
  ...
  LOOP
    ...
    CASE rec.proposal_family
      ...
      WHEN 'action' THEN
        -- Correction 283 : filtre regex avant le cast ::date pour éviter les erreurs
        -- sur les valeurs LLM non-ISO ("25 du mois en cours", "fin de mois", etc.)
        v_due_date := CASE
      ...
      WHEN 'decision' THEN
        INSERT INTO public.site_decisions (
          ...
          v_eff_label, v_eff_desc,
          COALESCE(
            CASE WHEN rec.source_payload->>'date_decision' ~ '^\d{4}-\d{2}-\d{2}$'
      ...
      WHEN 'deadline' THEN
        -- Correction 283 : même filtre regex pour les échéances.
        v_due_date := CASE
          WHEN COALESCE(rec.source_payload->>'dueDate', rec.source_payload->>'due_date')
               ~ '^\d{4}-\d{2}-\d{2}$'
          THEN (COALESCE(rec.source_payload->>'dueDate', rec.source_payload->>'due_date'))::date
...
COMMENT ON FUNCTION public.materialize_historical_visit(uuid, uuid, uuid, date, text) IS
  ...
  'Mig 283 : cast ::date protégé par filtre regex ISO pour les dates LLM non parsables.';
