...
BEGIN
  ...
  IF p_planned_point IS NOT NULL THEN
    ...
    IF p_planned_point->>'foundingKind' = 'cbo' THEN
      ...
      ELSE
        ...
        -- trackable_condition déjà fondé sur ce thread ?
    ...
    ELSE -- foundingKind = 'trackable_condition'
      ...
      ELSE
        ...
        -- fuzzy — jamais auto-lié même unique, cf. tête de fichier).
        SELECT array_agg(DISTINCT tp.id ORDER BY tp.id) INTO v_sibling_ids
        FROM public.tracked_point_member tpm
        JOIN public.tracked_point tp ON tp.id = tpm.tracked_point_id
        WHERE tpm.subject_thread_id = p_thread_id AND tpm.status = 'active'
        ...
        IF array_length(v_sibling_ids, 1) = 1 THEN
          SELECT * INTO v_target FROM public.tracked_point WHERE id = v_sibling_ids[1] FOR UPDATE;
          IF v_target.status <> 'active' OR v_target.identity_status = 'CONFLICTED' THEN
            -- Race : devenu invalide entre le scan et le verrou → traité comme 0
            -- cible thread-scoped exploitable, cf. ELSE ci-dessous.
            v_sibling_ids := NULL;
          END IF;
        END IF;
        ...
        ELSE
          ...
          -- réutilisé tel quel, aucun rail llm câblé). Jamais fait confiance en aveugle :
          -- seuls les Points encore actifs/non-CONFLICTED comptent.
          SELECT array_agg(DISTINCT tp.id ORDER BY tp.id) INTO v_cross_thread_ids
          FROM unnest(p_cross_thread_candidate_point_ids) AS cid
          JOIN public.tracked_point tp ON tp.id = cid
          WHERE tp.status = 'active' AND tp.identity_status <> 'CONFLICTED';
  ...
  ELSIF p_planned_pending_trace IS NOT NULL THEN
    ...
    -- Voisinage thread-scoped uniquement — cette branche ne fonde déjà jamais de
    -- Point automatiquement, D1 (cross-thread) n'y ajoute rien (cf. tête de fichier).
    SELECT array_agg(DISTINCT tp.id ORDER BY tp.id) INTO v_sibling_ids
    FROM public.tracked_point_member tpm
    JOIN public.tracked_point tp ON tp.id = tpm.tracked_point_id
    WHERE tpm.subject_thread_id = p_thread_id AND tpm.status = 'active'
