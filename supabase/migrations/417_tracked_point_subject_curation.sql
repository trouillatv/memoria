-- Migration 417 — curation humaine durable d'un Point vers un Sujet canonique.
--
-- Doctrine :
--   - un déplacement humain de Point n'est pas une retouche cosmétique de FK ;
--   - il devient la vérité de routage pour le thread historique, le Point, les CBO,
--     les objets métier matérialisés et les occurrences déjà projetées ;
--   - la décision est auditée et marquée `manual` pour survivre aux replay et au
--     matching automatique.

CREATE TABLE IF NOT EXISTS public.tracked_point_subject_override (
  id                            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  site_id                       UUID NOT NULL REFERENCES public.sites(id) ON DELETE CASCADE,
  tracked_point_id              UUID NOT NULL REFERENCES public.tracked_point(id) ON DELETE CASCADE,
  previous_canonical_subject_id UUID REFERENCES public.canonical_subject(id) ON DELETE SET NULL,
  target_canonical_subject_id   UUID NOT NULL REFERENCES public.canonical_subject(id) ON DELETE RESTRICT,
  subject_thread_ids            UUID[] NOT NULL,
  reason                        TEXT,
...
CREATE INDEX IF NOT EXISTS tracked_point_subject_override_site_idx
  ON public.tracked_point_subject_override(site_id);

CREATE INDEX IF NOT EXISTS tracked_point_subject_override_point_idx
  ON public.tracked_point_subject_override(tracked_point_id);

CREATE UNIQUE INDEX IF NOT EXISTS tracked_point_subject_override_active_uidx
  ON public.tracked_point_subject_override(tracked_point_id)
  WHERE superseded_at IS NULL;

ALTER TABLE public.tracked_point_subject_override ENABLE ROW LEVEL SECURITY;

CREATE POLICY "org members can view tracked_point_subject_override"
  ON public.tracked_point_subject_override FOR SELECT
  USING (
    site_id IN (
...
CREATE POLICY "service role manages tracked_point_subject_override"
  ON public.tracked_point_subject_override FOR ALL
