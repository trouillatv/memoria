-- Migration 411 : fn_reconcile_tracked_point_unit — redéploiement explicite du correctif 403
-- (P0-2A.1, audit READ-ONLY mandat Vincent — item 2 : dérive du tracker de migrations).
--
-- NO GO APPLY — cette migration n'est pas appliquée par ce lot (mandat explicite Vincent :
-- "HARD STOP avant toute migration en production"). Fichier écrit, non exécuté.
--
-- ── Pourquoi ce fichier existe (et pourquoi 403 n'est pas modifiée) ───────────────────────
--
-- Audit P0-2A.1 (item 2) : `_migrations_applied` enregistre 400/401/402/403 comme appliquées
-- (même lot, 2026-09-15 05:10:36 → 05:10:39 UTC, un événement à un an d'écart de leur écriture),
-- mais une vérification live (pg_proc.prosrc, md5 et pg_get_functiondef) confirme que le corps
-- RÉELLEMENT déployé de fn_reconcile_tracked_point_unit est encore exactement celui de la
-- migration 401 : le fast-path NOOP d'ATTACH_MEMBER n'exige toujours qu'"une membership active
-- quelconque existe pour ce thread" (aucune variable v_snapshot_scope/v_snapshot_proposal_ids
-- présente en base), pas la composition scope+proposal_ids exacte que 403 introduit. La
-- correction de 403 n'a donc jamais atteint la base malgré le tracker.
--
-- Mécanisme le plus probable (non prouvé à 100 % — aucun log serveur Postgres accessible pour
-- l'instant précis 2026-09-15 05:10:39 UTC via l'API Management, à documenter comme limite
-- connue) : soit l'heuristique "déjà existant ⇒ marquer comme appliqué sans revérifier le
-- contenu live" documentée par scripts/_db-push-reconcile-once.ts pour des cas antérieurs
-- similaires (292, 317, 328-332), soit un rollback transactionnel implicite Postgres sur
-- l'ensemble multi-instructions du fichier 403 (chaque migration est envoyée comme une seule
-- chaîne de requêtes ; l'échec d'une instruction TARDIVE — COMMENT ON FUNCTION, REVOKE, GRANT —
-- aurait annulé y compris le CREATE OR REPLACE FUNCTION apparemment réussi plus haut dans la
-- même chaîne). Dans les deux cas, `_migrations_applied` s'est retrouvée fausse SANS qu'aucune
-- instruction supplémentaire ne soit nécessaire pour l'expliquer côté application.
--
-- Décision Vincent (préférence explicite) : ne JAMAIS corriger `_migrations_applied` a
-- posteriori ni ré-appliquer 403 telle quelle — cela réécrirait l'historique d'audit pour le
-- faire paraître artificiellement propre. À la place, cette migration 411, strictement
-- postérieure et monotone, redéploie EXPLICITEMENT la définition corrigée de la fonction. Le
-- corps ci-dessous est repris à l'identique de 403 (aucune modification fonctionnelle
-- supplémentaire, aucun changement au-delà de ce que 403 documentait déjà) — seul le fichier et
-- son commentaire de tête diffèrent, pour que l'historique Git/migrations reflète fidèlement
-- que 403 a été écrite, marquée appliquée à tort, puis effectivement redéployée par 411.
--
-- Preuve exécutable de la nécessité de ce redéploiement (item 3 du mandat) : test "Témoin 27"
-- (tests/lib/db/tracked-point-live-writer.test.ts) — un appel RPC direct forçant une collision
-- de fingerprint sur [A]→[A,B] (scope=proposal_set) reproduit AUJOURD'HUI, contre la 401 live
-- inchangée, la perte silencieuse de B dans tracked_point_member (write_pattern=NOOP à tort).
-- Ce même test devra être mis à jour pour refléter le comportement corrigé une fois 411
-- effectivement appliquée (revalidation complète attendue au lieu du NOOP).
--
-- Live Writer : allowlist TRACKED_POINT_LIVE_WRITER_SITE_IDS toujours vide (OFF) au moment de
-- l'écriture de ce fichier. Aucune donnée de production touchée par ce lot. Audit OCEF (item 1)
-- : 0 événement scope=proposal_set en production à ce jour — aucune réparation de donnée requise.
...
DECLARE
  ...
  -- Migration 403 (redéployée par 411) : composition attendue par l'unité courante, extraite
  -- de p_input_snapshot, pour le fast-path ATTACH_MEMBER exact ci-dessous (§4). Triée pour
  -- comparer un ENSEMBLE, pas une séquence (même convention que buildInputSnapshot côté TS).
...
BEGIN
  ...
  IF FOUND AND v_prev_state.input_fingerprint = p_input_fingerprint THEN
    ...
    -- Migration 403 (redéployée par 411) : ATTACH_MEMBER exige désormais une membership
    -- active dont le scope et, pour proposal_set, la composition canonique (triée) de
    -- proposal_ids correspondent EXACTEMENT à ce que l'unité courante attend
    -- (p_input_snapshot) — défense en profondeur, la protection principale reste le
    -- fingerprint (qui inclut désormais proposalIds côté TS et ne matche déjà plus si la
    -- composition a changé). ENRICH_EXISTING_POINT reste sur l'ancien contrat inchangé (voir
    -- commentaire de tête de migration 403).
