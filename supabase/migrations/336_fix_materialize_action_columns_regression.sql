-- Migration 336 — Corrige la régression du bloc WHEN 'action' de
-- materialize_historical_visit(), réintroduite par la migration 298.
--
-- Erreur observée à l'import d'un PV historique (bouton "Créer la visite
-- historique") : column "description" of relation "site_actions" does not
-- exist.
--
-- La migration 273 avait déjà corrigé ce bloc (description→body,
-- deadline→due_date, issued_by→assigned_to). La migration 298 a réécrit la
-- fonction en entier pour ajouter la matérialisation des photos, mais a
-- recopié une version antérieure (pré-273) du bloc 'action' — la régression
-- est passée inaperçue car aucun test n'appelle ce chemin avec une
-- proposition de famille 'action'.
--
-- Cette migration ne touche que le bloc WHEN 'action' ; le reste de la
-- fonction (dont la boucle photos ajoutée en 298) est repris à l'identique.
...
COMMENT ON FUNCTION public.materialize_historical_visit IS
  ...
  '(336 : corrige la régression 298 sur le bloc action — body/due_date/assigned_to)';
