-- Migration 338 — Corrige les blocs WHEN 'deadline', WHEN 'observation' et
-- WHEN 'decision' de materialize_historical_visit().
--
-- Erreur observée (deadline) : column "description" of relation
-- "site_deadlines" does not exist.
--
-- La migration 274 avait déjà corrigé deadline et observation :
--   deadline    : description → constraint_text, status 'pending' → 'to_plan'
--                 (CHECK status IN ('to_plan','planned','done','cancelled'))
--   observation : description → body, raised_at/raised_by → confirmed_by
--                 (site_watchpoints n'a ni description ni raised_at/raised_by)
-- La migration 298 a de nouveau recopié une version antérieure (pré-274) de
-- ces deux blocs en réécrivant la fonction pour ajouter la matérialisation
-- des photos — même régression que celle déjà corrigée pour le bloc action
-- (migration 336) et pour document_proposal_materialization (migration 337).
--
-- Le bloc observation n'a pas encore été vu en échec en production (l'ordre
-- de traitement des propositions dans cette visite a atteint 'deadline'
-- avant 'observation'), mais le bug est prouvé par la définition de table
-- réelle (site_watchpoints.body/confirmed_by, pas de description ni
-- raised_at/raised_by) : corrigé préventivement dans la même migration.
--
-- Le bloc decision a le même problème mais n'a jamais été corrigé par une
-- migration antérieure : site_decisions utilise des noms de colonnes en
-- français (titre/statut/date_decision), vérifiés directement via
-- information_schema sur la base de production, alors que la fonction
-- écrit title/status/decided_at (colonnes inexistantes). 'actee' (valeur
-- par défaut de statut, CHECK site_decisions_statut_check) est correct pour
-- une décision actée dans un PV historique déjà signé.
--
-- Cette migration ne touche que ces trois blocs ; le reste de la fonction
-- (dont les fixes 336 et 337) est repris à l'identique.
...
COMMENT ON FUNCTION public.materialize_historical_visit IS
  ...
  '(338 : corrige deadline constraint_text/to_plan, observation body/confirmed_by '
  'et decision titre/statut/date_decision)';
