-- Migration 274 — Correction colonnes site_watchpoints et site_deadlines
--
-- Erreurs détectées dans la fonction materialize_historical_visit() :
--
--   WHEN 'observation' :
--     description → body  (site_watchpoints.description n'existe pas)
--     raised_at, raised_by → n'existent pas → confirmed_by (uuid, p_user_id)
--
--   WHEN 'deadline' :
--     description → constraint_text  (site_deadlines.description n'existe pas)
--     'pending'   → 'to_plan'        (CHECK constraint : to_plan/planned/done/cancelled)
...
BEGIN
  ...
  INSERT INTO public.document_links (document_id, target_type, target_id)
    VALUES (v_doc_id, 'site_report', v_report_id)
    ...
    CASE rec.proposal_family
      ...
      WHEN 'reservation' THEN
        ...
        INSERT INTO public.document_proposal_materialization (
          ...
          v_org_id, rec.id, 'site_reserve', v_entity_id, p_user_id
      ...
      WHEN 'action' THEN
        ...
        INSERT INTO public.document_proposal_materialization (
          ...
          v_org_id, rec.id, 'site_action', v_entity_id, p_user_id
        ) ON CONFLICT DO NOTHING;
      ...
      WHEN 'decision' THEN
        ...
        INSERT INTO public.document_proposal_materialization (
          ...
          v_org_id, rec.id, 'site_decision', v_entity_id, p_user_id
        ) ON CONFLICT DO NOTHING;
      ...
      WHEN 'observation' THEN
        ...
        INSERT INTO public.document_proposal_materialization (
          ...
          v_org_id, rec.id, 'site_watchpoint', v_entity_id, p_user_id
        ) ON CONFLICT DO NOTHING;
      ...
      WHEN 'deadline' THEN
        ...
        INSERT INTO public.document_proposal_materialization (
          ...
          v_org_id, rec.id, 'site_deadline', v_entity_id, p_user_id
        ) ON CONFLICT DO NOTHING;
  ...
  -- ── knowledge_fact en attente (pipeline TypeScript post-RPC) ──────────────
  SELECT EXISTS (
    SELECT 1
      FROM public.document_extraction_proposal
      WHERE extraction_run_id = p_run_id
...
COMMENT ON FUNCTION public.materialize_historical_visit IS
  ...
  '(274 : correction body/constraint_text/to_plan/confirmed_by)';
