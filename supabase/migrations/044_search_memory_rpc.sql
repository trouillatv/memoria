-- Migration 044 — RPC search_memory (Phase 2.2).
--
-- Interroge les 4 sources FTS indexées en 043 et retourne une liste typée
-- pour alimenter l'overlay de recherche global. Tri par pertinence (ts_rank)
-- décroissante puis date décroissante.
...
grant execute on function public.search_memory(text, uuid, uuid, int, int) to authenticated;
grant execute on function public.search_memory(text, uuid, uuid, int, int) to service_role;
