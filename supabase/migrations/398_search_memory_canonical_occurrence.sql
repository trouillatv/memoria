-- Migration 398 — GO MEMORY SEARCH CANONICAL OCCURRENCE.
--
-- Audit MEMORY-SEARCH TRUTH (2026-09-08, verdict PARTIAL_TRUTH_GAP) : une preuve
-- canonique (canonical_subject_occurrence) peut exister sans AUCUN équivalent
-- legacy indexé (ex. field_visit sans site_knowledge_proposals cherchable) et
-- reste alors invisible dans search_memory. Cette migration ferme ce trou pour
-- l'occurrence elle-même, SANS prétendre indexer canonical_subject ou
-- tracked_point directement (hors périmètre, cf. mandat).
--
...
-- de nature différente du texte brut du compte-rendu (meeting_hits/pv_hits) ou
-- de l'observation terrain source (observation_hits sur visit_capture) — même
...
-- (visit_status/state_status/source_kind sont des énumérations techniques,
-- jamais indexées comme témoignage).
--
-- Position temporelle : coalesce(event_date, effective_date), doctrine mig 363
-- (« position temporelle longitudinale = COALESCE(event_date, effective_date) »).
--
-- Additive et idempotente. Rollback : recréer la RPC de la migration 387.
...
as $$
  ...
  ,canonical_occurrence_hits as (
    select
      ...
      co.id,
      co.label as title,
      coalesce(nullif(co.note, ''), co.label) as snippet,
      coalesce(co.event_date, co.effective_date)::timestamptz as occurred_at,
      co.site_id,
      s.contract_id,
      ts_rank(
        to_tsvector('public.french_unaccent', concat_ws(' ', co.label, coalesce(co.note, ''))),
        q.tsq) as rank,
      null::uuid as subject_id,
      co.source_ref_id as ref_id
    ...
    where co.source_kind = 'field_visit'
      ...
      and coalesce(co.event_date, co.effective_date)::timestamptz
            > now() - (q.days_window || ' days')::interval
      and (p_contract_id is null or s.contract_id = p_contract_id)
      and (p_site_id is null or co.site_id = p_site_id)
      and (p_org_id is null or s.organization_id = p_org_id)
  )
  ...
  union all select * from canonical_occurrence_hits
