...
function makeFakeSupabase(activeSubjects: Row[], opts: { insertShouldConflict: boolean; insertErrorCode?: string }) {
  return {
    from: (table: string) => {
      ...
      const api = {
        ...
        single: async () => {
          if (opts.insertShouldConflict) {
            return { data: null, error: { code: opts.insertErrorCode ?? '23505', message: 'duplicate key value violates unique constraint' } }
          }
