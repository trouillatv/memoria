...
describe('analyzeTender (mock provider)', () => {
  ...
  it('retourne reading + memo + score quand library vide', async () => {
    ...
    // Le prompt Lecteur AO a été itéré (LECTEUR_AO_V1.version = 'v4' —
    // vrais repères de page + 3 niveaux de confiance, fc1d1f2).
    expect(result.promptVersions.lecteur_ao).toBe('v4')
  })
})
