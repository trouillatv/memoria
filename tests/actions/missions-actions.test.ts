...
beforeEach(() => {
  ...
  mocks.requireOwned.mockResolvedValue({ allowed: true })
...
describe('createMissionAction — recalcul serveur de la population (FIX #1)', () => {
  ...
  it('refuse la création si requireOwned refuse le site (fail-closed)', async () => {
    mocks.requireOwned.mockResolvedValue({ allowed: false, error: 'Forbidden' })
...
describe('updateMissionAction — recalcul serveur + préservation des liens historiques (FIX #1)', () => {
  ...
  it('autorise CETTE mutation via requireOwned sur le site de la Mission — jamais le seul rôle plateforme (FIX_REQUIRED #1, revue finale)', async () => {
    mocks.getMission.mockResolvedValue({
      id: missionId,
      site_id: siteId,
      engagement_ids: [],
    ...
    expect(mocks.requireOwned).toHaveBeenCalledWith('manager', 'sites', siteId)
  })

  it('refuse la mise à jour si requireOwned refuse le site de la Mission (fail-closed), même sans engagement_ids soumis', async () => {
    mocks.getMission.mockResolvedValue({
      id: missionId,
      site_id: siteId,
      engagement_ids: [engOldPreserved],
