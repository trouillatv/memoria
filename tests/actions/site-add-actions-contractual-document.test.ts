...
// uploadSiteContractualDocumentAction est une action DÉDIÉE, distincte du
// flux générique "Document PDF" (uploadSiteDocumentAction). Le flux
// générique repliait silencieusement un document_type absent/invalide sur
// 'preuve' — c'est exactement ce repli qui a produit un CCTP importé comme
// "preuve" en recette (review ChatGPT sur le rapport P0-1, 2026-09-23).
// Cette action-ci ne connaît AUCUN repli : un type absent ou hors de la
// liste contractuelle (cctp/ccap/ordre_service/contrat/avenant/autre) est un
// refus immédiat, sans aucun effet de bord.
//
...
// vérifie que l'appartenance, jamais le rôle contextuel. La même garde
// (requireOrganizationRole AVANT tout effet de bord) est reconduite ici.
...
const { uploadSiteContractualDocumentAction } = await import('@/app/(dashboard)/sites/[id]/site-add-actions')
...
function contractualFormData(documentType?: string, effectiveDate?: string): FormData {
  ...
  if (documentType !== undefined) fd.set('document_type', documentType)
...
describe('uploadSiteContractualDocumentAction — nature du document', () => {
  it('transmet document_type=cctp tel quel, jamais réécrit', async () => {
    const result = await uploadSiteContractualDocumentAction(AGP_SITE, contractualFormData('cctp'))
  ...
  it('force target_type=site et target_id=chantier courant (aucun sélecteur de cible générique)', async () => {
    await uploadSiteContractualDocumentAction(AGP_SITE, contractualFormData('ccap'))
    const sentFd = uploadDocumentAction.mock.calls[0]![0] as FormData
    expect(sentFd.get('target_type')).toBe('site')
    expect(sentFd.get('target_id')).toBe(AGP_SITE)
  })

  it('transmet la date d’effet quand fournie', async () => {
    await uploadSiteContractualDocumentAction(AGP_SITE, contractualFormData('ordre_service', '2026-09-01'))
    const sentFd = uploadDocumentAction.mock.calls[0]![0] as FormData
  ...
  it('n’envoie pas effective_date quand absente (pas de champ vide parasite)', async () => {
    await uploadSiteContractualDocumentAction(AGP_SITE, contractualFormData('cctp'))
    const sentFd = uploadDocumentAction.mock.calls[0]![0] as FormData
    expect(sentFd.get('effective_date')).toBeNull()
  })

  it('document_type absent → refus immédiat, aucune création, aucun appel DB', async () => {
    const result = await uploadSiteContractualDocumentAction(AGP_SITE, contractualFormData(undefined))
  ...
  it('document_type="preuve" envoyé au flux contractuel → refus, jamais de repli silencieux', async () => {
    const result = await uploadSiteContractualDocumentAction(AGP_SITE, contractualFormData('preuve'))
    expect(result.ok).toBe(false)
    expect(uploadDocumentAction).not.toHaveBeenCalled()
  })

  it('document_type hors liste contractuelle (ex. "ao") → refus, aucun effet de bord', async () => {
    const result = await uploadSiteContractualDocumentAction(AGP_SITE, contractualFormData('ao'))
    expect(result.ok).toBe(false)
    expect(createDocumentCollection).not.toHaveBeenCalled()
    expect(uploadDocumentAction).not.toHaveBeenCalled()
  })
...
describe('uploadSiteContractualDocumentAction — garde rôle AVANT effet de bord', () => {
  ...
  it('la garde repose sur le rôle CONTEXTUEL (chantier), jamais un rôle global : chef_equipe côté chantier refuse même si getUserRoleById répondait manager', async () => {
    ...
    // consulté par uploadSiteContractualDocumentAction : seul
    // requireOrganizationRole (rôle DANS l'organisation du chantier) fait
    // autorité.
    const result = await uploadSiteContractualDocumentAction(AGP_SITE, contractualFormData('cctp'))
    expect(result.ok).toBe(false)
  })
