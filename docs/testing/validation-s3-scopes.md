...
✅ **Réussite (la personne découvre le besoin elle-même)** — elle répond
naturellement « j'irais dans le VRD », « dans tout ce qui touche au VRD », « la
mémoire du VRD ». Elle n'a pas eu besoin qu'on lui montre l'outil : elle a inventé
le rangement adressable parce que la douleur l'exige.
❌ **Échec** — « j'appellerais quelqu'un », « je fouillerais les mails / les
photos », « je redemanderais à Jean », « aucune idée ». Le besoin d'adressage n'est
pas ressenti ; l'outil arrivera dans le vide.
...
| Critère | P1 | P2 | P3 |
|---|:--:|:--:|:--:|
| ★★ **A DÉCOUVERT le besoin seul** (Temps 1, Test A/B — signal le plus fort) | ☐ | ☐ | ☐ |
...
Deux niveaux, par ordre de force :

1. **Signal fort (Temps 1)** — au moins 2 personnes **découvrent le besoin
   elles-mêmes** : mises devant la douleur (Tests A/B), elles vont spontanément
   « dans le VRD » sans qu'on ait montré l'outil. C'est le signal le plus
   défendable : le besoin existe *avant* la feature.
2. **Confirmation (Temps 2)** — ces mêmes personnes, une fois l'écran révélé,
   lâchent une **phrase de valeur spontanée** (« ça évite de chercher dans tout le
   chantier », « je retrouve tout ce qui concerne le VRD », « quand quelqu'un part
   on garde tout au même endroit ») **et** le Test 6 montre une dépendance réelle.

**S3 est VALIDÉ** si ≥2 personnes donnent le signal fort **et** la confirmation.
Une coche d'interface ne suffit pas : sans découverte du besoin (Temps 1) ni phrase
de valeur, considérer S3 comme **non validé** — on aura prouvé la lisibilité, pas
l'utilité.
