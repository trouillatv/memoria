...
| **Plan 3.6 — Atelier IA multi-agents métier (analyses + challenge)** | 📝 prévu | **Scope élargi** par rapport au draft initial. Inclut : (a) **analyse initiale propre à chaque agent** sur l'AO (table `tender_agent_analyses`, exécutée à la demande depuis la sidebar) ; (b) sidebar avec liste des 7 agents + badges d'état (« analyse faite », « risque détecté », « challenge actif ») ; (c) **multi-select max 3 agents** via checkbox dans la composer ; (d) réponses **parallèles** (Promise.all de `chatWithAgent`) avec bulles séparées par agent ; (e) bouton **« Lancer un challenge »** pour 1 round où chaque agent voit et réagit aux réponses des autres ; (f) **max 2 rounds de challenge** au total. UX : thread style Slack/ChatGPT, avatar + spécialité par bulle, séparateurs visuels par round. **Coûts maîtrisés** : N≤3 agents en parallèle, ≤2 challenges par turn. **Pas** de LangGraph, pas d'autonomie agentique. |
| Plan 3.7+ — orchestration avancée | 🟡 idée | Réservé si l'usage de 3.6 montre un vrai besoin. Pourrait inclure : streaming par agent, persona personnalisée, mémoire long terme cross-tender, agent `stratege_commercial`. **Hors scope MVP.** |
