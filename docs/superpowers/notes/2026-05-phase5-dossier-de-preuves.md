...
(pas seulement quand un AO sort). C'est la fonctionnalité qui transforme NetoIAge
...
- PDF generator via `@react-pdf/renderer` — header tenant + footer fixe avec QR code + watermark NetoIAge
...
- Pas de "à propos NetoIAge" sur la page publique (pas encore de page about marketing — décision assumée).
