...
**5. Pas de logo client de Patrick.** L'entreprise de Patrick, « NetCal », n'apparaît qu'en sous-titre minuscule (« tenantName · Dossier de preuves »). Pour Sylvie, le document **n'est pas émis par Patrick** — il est émis par NetoIAge. Inversion de la relation. Sylvie n'est pas cliente de NetoIAge, elle est cliente de NetCal.
...
**8. Aucune réponse possible côté Sylvie.** Elle voudrait dire « j'ai bien reçu » ou « pourrais-tu compléter avec X ». Aujourd'hui, elle doit retourner sur Outlook, écrire un mail à Patrick. Friction. Le lien NetoIAge l'a fait sortir de son flux mail pour rien — elle y retourne.
...
## 6. Dépendances externes — NetoIAge n'absorbe rien
...
NetoIAge doit **cohabiter** avec : Outlook (transport), imprimante (archive physique), arborescence locale (archive numérique), réunion orale (présentation), forwarding (transmission interne). **Ne jamais vouloir absorber ces flux.** Le lien NetoIAge est un **point de vérité**, pas un canal de communication.

## 7. Risques doctrinaux — pressions futures

Sylvie, sous trois mois de pratique, va envoyer des demandes à Patrick (qui les remontera à NetoIAge). Chaque demande doit avoir une réponse doctrinale **préparée**.

| Demande probable | Risque doctrinal | Réponse |
|---|---|---|
| « Patrick, fais en sorte que ton outil m'envoie le rapport direct par mail le 5 du mois » | Devenir émetteur à la place du prestataire | ❌ Refus. Patrick reste l'expéditeur. NetoIAge ne mail jamais le client final. Sinon NetoIAge devient l'interlocuteur. Dérive vers SaaS B2B2C — le client se fidélise à la plateforme, pas au prestataire. |
| « Je veux commenter directement sur le document » | Portail client = grosse dérive | ❌ Refus. Pas de commentaires inline. Sylvie répond à Patrick par mail. NetoIAge n'est pas une messagerie. |
| « Je veux voir l'historique cumulé de tous les rapports de Patrick » | Portail client permanent avec login | ❌ Refus. Un lien = un document. Si Sylvie veut un historique, elle doit demander à Patrick (le prestataire reste l'interlocuteur). Si on cède : NetoIAge devient un mini-portail, le moat de « scope assumé » s'effrite. |
...
| « Je veux noter Patrick » | Rating = mesure individuelle | ❌ Refus net. NetoIAge ne fera **jamais** de système de note client. Glissement immédiat vers KPI fournisseur, comparaison, classement. C'est la fin du produit. Sylvie a Excel pour ça. |
...
| « NetoIAge envoie un mail à Sylvie automatiquement le 5 du mois » | NetoIAge devient l'auteur | ❌ Refus. Patrick reste celui qui clique « envoyer ». Pas d'automatisation transparente. Le rapport mensuel doit avoir un instant humain de validation. |
| « Je veux une API pour intégrer NetoIAge à mon ERP hospitalier » | Dérive plateforme | ❌ Refus. Export PDF + ICS suffit. Pas d'API client. Si l'hôpital veut intégrer, il scrape le PDF côté ERP. NetoIAge n'est pas un fournisseur de données structurées pour SI tiers. |
| « Sylvie demande à recevoir une alerte SMS dès qu'une anomalie est résolue sur son site » | Notification client | ❌ Refus. Pas de canal direct NetoIAge → client final. Tout passe par Patrick. |
...
**Principe transversal** : *Sylvie consulte. Sylvie n'opère pas. NetoIAge est le canal de Patrick vers Sylvie, jamais le serviteur de Sylvie.*
...
**Co-branding sobre prestataire + plateforme.** En haut du PDF et de la page web : « *Document émis par NetCal · Plateforme de preuves NetoIAge* ». Logo de l'entreprise Patrick (NetCal) à gauche, marque NetoIAge à droite, séparateur fin. Permet à Sylvie de reconnaître Patrick instantanément, tout en gardant la signature de tiers. **Sylvie est cliente de NetCal, pas de NetoIAge** — l'UI doit refléter ça.
...
**Watermark + URL lisible en clair.** Sur chaque page du PDF, en pied : « *Vérifiable : netoiage.app/v/X9K2-AB3F* ». Plus utile que le QR pour un humain en réunion. Le QR reste pour les auditeurs qui scannent depuis un bureau.

**Signature cryptographique du PDF.** Le PDF est signé numériquement par NetoIAge au moment de la génération. Adobe Reader affiche un bandeau « ce document est signé et n'a pas été modifié ». Sylvie ne verra peut-être pas, mais ça vaut pour les auditeurs. Coût technique faible (PAdES).
...
- **Note / rating de la prestation par Sylvie** → glissement immédiat vers KPI fournisseur, comparaison de Patrick avec d'autres prestataires NetoIAge. Refus net, irréversible.
...
- **Forwarding automatique à la direction de Sylvie** → Sylvie reste maître de la communication interne. NetoIAge n'envoie pas de mail.
...
*Effet* : Sylvie reconnaît Patrick immédiatement, le document n'est plus « émis par NetoIAge » mais « émis par NetCal via NetoIAge ». Restaure la relation commerciale réelle. Augmente le sentiment d'autorité sans verser dans le marketing.
*Effort* : faible (1-2 jours). Champ `tenant.logo_url` + render dans le header PDF et la page publique.
*Garde-fou doctrinal* : logo prestataire reste sobre (taille modeste), pas de slogan, pas de couleurs envahissantes. La marque NetoIAge reste visible comme signature de tiers.
...
*Effort* : moyen-fort (5-8 jours). Route publique `/v/[hash]` qui ne fait que confirmer « ce hash de PDF a été émis par NetoIAge le X par tenant Y ». Pas de données preuve revealed, juste un certificat d'intégrité. Décorréler de la table `share_tokens`.
