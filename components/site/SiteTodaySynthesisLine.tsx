...
export function SiteTodaySynthesisLine({ synthesis }: { synthesis: SiteTodaySynthesis }) {
  if (synthesis.totalPoints === 0) {
    return <p className="text-sm text-muted-foreground">Rien de particulier à signaler aujourd&apos;hui.</p>
  }

  const parts: string[] = [`${synthesis.totalPoints} Point${synthesis.totalPoints > 1 ? 's' : ''}`]
  if (synthesis.openPoints > 0) parts.push(`${synthesis.openPoints} ouvert${synthesis.openPoints > 1 ? 's' : ''}`)
  if (synthesis.reopenedPoints > 0) parts.push(`${synthesis.reopenedPoints} réouvert${synthesis.reopenedPoints > 1 ? 's' : ''}`)
  if (synthesis.resolvedRecently > 0) parts.push(`${synthesis.resolvedRecently} résolu${synthesis.resolvedRecently > 1 ? 's' : ''}`)
  if (synthesis.needsYouCount > 0) parts.push(`${synthesis.needsYouCount} question${synthesis.needsYouCount > 1 ? 's' : ''} pour MemorIA`)
  if (synthesis.lastActivityAt) parts.push(`dernière activité ${dateFmt.format(new Date(synthesis.lastActivityAt))}`)
