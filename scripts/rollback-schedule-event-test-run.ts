// Rollback du harnais de recette réversible P5-F3 (SCHEDULE_VISIT + SCHEDULE_MEETING).
...
//
// Rollback = deleted_at (soft delete), PAS status='cancelled' : ce statut porte
// un sens métier réel (annulation décidée par un humain, mig 216) que ce
// harnais de nettoyage ne doit pas détourner. deleted_at est déjà exclu de
// toutes les lectures (lib/db/scheduled-events.ts filtre `.is('deleted_at',
// null)`) — même discipline que rollback-knowledge-correction-test-run.ts (P5-F2b).
//
// Usage : npx tsx scripts/rollback-schedule-event-test-run.ts <testRunId>
...
async function main() {
  ...
  if (!testRunId) {
    console.error('Usage: npx tsx scripts/rollback-schedule-event-test-run.ts <testRunId>')
    process.exit(1)
  }
  ...
  const ids = manifest.entries.filter((e) => e.table === 'site_scheduled_events').map((e) => e.id)

  console.log(`── Rollback recette P5-F3 (SCHEDULE_VISIT + SCHEDULE_MEETING) — testRunId=${testRunId} ──`)
  console.log(`Manifeste : ${manifestPath}`)
  console.log(`siteId = ${manifest.siteId} (PETRO)\n`)

  if (ids.length === 0) {
    console.log('Aucune ligne site_scheduled_events dans ce manifeste — rien à annuler.')
    process.exit(0)
  }
  ...
  const { data: current, error: readError } = await supabase
    ...
    .select('id, site_id, type, title, status, deleted_at, copilot_proposal_id')
    .in('id', ids)
  if (readError) {
  ...
  for (const row of current ?? []) {
    ...
    console.log(`  - id=${row.id} type=${row.type} statut=${row.status} title="${row.title}" (phrase: "${entry?.phrase ?? '?'}")`)
  ...
  const { data: after } = await supabase.from('site_scheduled_events').select('id, deleted_at').in('id', safeIds)
