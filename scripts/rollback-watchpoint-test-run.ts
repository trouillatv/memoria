// Rollback du harnais de recette réversible P4-E1 (site_watchpoints).
...
//
// Rollback = SUPPRESSION DURE, PAS deleted_at : site_watchpoints a bien une
// colonne deleted_at (mig 217), mais ce harnais choisit la suppression dure —
// même discipline que rollback-relation-claim-test-run.ts (P4-D1) — pour
// garantir une restauration RÉELLE de l'état initial plutôt que d'accumuler
// des lignes de test soft-deleted en base indéfiniment.
//
...
// titre ou par plage de dates (qui pourrait toucher un vrai point de vigilance
// créé en production entre-temps).
//
// Usage : npx tsx scripts/rollback-watchpoint-test-run.ts <testRunId>
...
async function main() {
  ...
  if (!testRunId) {
    console.error('Usage: npx tsx scripts/rollback-watchpoint-test-run.ts <testRunId>')
    process.exit(1)
  }
  ...
  const ids = manifest.entries.filter((e) => e.table === 'site_watchpoints').map((e) => e.id)

  console.log(`── Rollback recette P4-E1 — testRunId=${testRunId} ──`)
  console.log(`Manifeste : ${manifestPath}`)
  console.log(`siteId = ${manifest.siteId} (PETRO)\n`)

  if (ids.length === 0) {
    console.log('Aucune ligne site_watchpoints dans ce manifeste — rien à annuler.')
    process.exit(0)
  }
  ...
  const { data: after } = await supabase.from('site_watchpoints').select('id').in('id', safeIds)
