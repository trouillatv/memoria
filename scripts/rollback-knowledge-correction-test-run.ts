// Rollback du harnais de recette réversible P5-F2b (CORRECTION_KNOWLEDGE).
// (Vincent, 2026-08-17.) Annule UNIQUEMENT les lignes créées par un
// test_run_id donné, identifiées par leur ID exact dans le manifeste — jamais
// par titre ou date approximative. Même discipline que
// rollback-knowledge-lifecycle-test-run.ts (P5-F2a).
//
// Rollback = deleted_at (soft delete), PAS status='archived'/'superseded' :
// ces deux statuts portent un sens métier réel (mig 218, 334) que ce harnais
// de nettoyage ne doit pas détourner. deleted_at est déjà exclu de toutes les
// lectures (listKnowledgeEntries, listRecentCurrentInformationEntries filtrent
// `.is('deleted_at', null)`).
//
// Usage : npx tsx scripts/rollback-knowledge-correction-test-run.ts <testRunId>
...
async function main() {
  ...
  if (!testRunId) {
    console.error('Usage: npx tsx scripts/rollback-knowledge-correction-test-run.ts <testRunId>')
    process.exit(1)
  }
  ...
  console.log(`── Rollback recette P5-F2b — testRunId=${testRunId} ──`)
