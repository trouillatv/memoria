// Rollback du harnais de recette réversible P4-E3 (site_reserve).
...
//
// Rollback = SUPPRESSION DURE : site_reserve n'a pas de colonne deleted_at
// (contrairement à site_deadlines) — même discipline que
// rollback-watchpoint-test-run.ts (P4-E1) pour garantir une restauration
// RÉELLE de l'état initial.
//
...
// libellé ou par plage de dates (qui pourrait toucher une vraie réserve créée
// en production entre-temps).
//
// Usage : npx tsx scripts/rollback-reserve-test-run.ts <testRunId>
...
async function main() {
  ...
  if (!testRunId) {
    console.error('Usage: npx tsx scripts/rollback-reserve-test-run.ts <testRunId>')
    process.exit(1)
  }
  ...
  const ids = manifest.entries.filter((e) => e.table === 'site_reserve').map((e) => e.id)

  console.log(`── Rollback recette P4-E3 — testRunId=${testRunId} ──`)
  console.log(`Manifeste : ${manifestPath}`)
  console.log(`siteId = ${manifest.siteId} (PETRO)\n`)

  if (ids.length === 0) {
    console.log('Aucune ligne site_reserve dans ce manifeste — rien à annuler.')
    process.exit(0)
  }
  ...
  const { data: current, error: readError } = await supabase
    .from('site_reserve')
    .select('id, site_id, label, status, copilot_proposal_id')
    .in('id', ids)
  if (readError) {
  ...
  for (const row of current ?? []) {
    ...
    console.log(`  - id=${row.id} statut=${row.status} label="${row.label}" (phrase: "${entry?.phrase ?? '?'}")`)
  ...
  const { data: after } = await supabase.from('site_reserve').select('id').in('id', safeIds)
