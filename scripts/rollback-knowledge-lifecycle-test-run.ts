// Rollback du harnais de recette réversible P5-F2a (site_knowledge_entries,
// archivage + supersession). (Vincent, 2026-08-17.) Annule UNIQUEMENT les
// lignes créées par un test_run_id donné, identifiées par leur ID exact dans le
// manifeste — jamais par titre ou date approximative.
//
// Rollback = deleted_at (soft delete), PAS status='archived'/'superseded' :
// ces deux statuts portent un sens métier réel (mig 218, 334) que ce harnais de
// nettoyage ne doit pas détourner. deleted_at est déjà exclu de toutes les
// lectures (listKnowledgeEntries filtre `.is('deleted_at', null)`).
//
// Le manifeste contient à la fois les entrées créées directement
// (createKnowledgeEntry) et celles créées par la RPC de supersession
// (supersede_knowledge_entry) — toutes deux nettoyées de la même façon.
//
// Usage : npx tsx scripts/rollback-knowledge-lifecycle-test-run.ts <testRunId>
...
async function main() {
  ...
  if (!testRunId) {
    console.error('Usage: npx tsx scripts/rollback-knowledge-lifecycle-test-run.ts <testRunId>')
    process.exit(1)
  }
  ...
  console.log(`── Rollback recette P5-F2a — testRunId=${testRunId} ──`)
  ...
  for (const row of current ?? []) {
    ...
    console.log(`  - id=${row.id} titre="${row.title}" kind=${row.kind} statut=${row.status} (note: "${entry?.note ?? '?'}")`)
